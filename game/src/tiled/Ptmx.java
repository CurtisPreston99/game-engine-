...
public class Ptmx {
	...
	private int camwidth, camheight, camleft, camtop, mapwidth, mapheight, tilewidth, tileheight, staggerindex,
			hexsidelength, backgroundcolor;
	...
	private class OrthogonalMap extends Xmap {
		...
		public void drawTileLayer(PGraphics pg, int m) {
			...
			int xstart = parent.max(0,
					parent.floor(this.canvasToMap(Ptmx.this.camleft, Ptmx.this.camtop).x - Ptmx.this.drawmargin));
			int xstop = parent.min(Ptmx.this.mapwidth,
					parent.ceil(this.canvasToMap(Ptmx.this.camleft + pg.width, Ptmx.this.camtop + pg.height).x
							+ Ptmx.this.drawmargin));
			int ystart = parent.max(0,
					parent.floor(this.canvasToMap(Ptmx.this.camleft, Ptmx.this.camtop).y - Ptmx.this.drawmargin));
			int ystop = parent.min(Ptmx.this.mapheight,
					parent.ceil(this.canvasToMap(Ptmx.this.camleft + pg.width, Ptmx.this.camtop + pg.height).y
							+ Ptmx.this.drawmargin));
			for (int ny = ystart; ny < ystop; ny++)
				for (int nx = xstart; nx < xstop; nx++) {
					...
					if (n >= 0) {
						...
						y = p.y - Ptmx.this.tileheight / 2 + tileN.offsety + l.offsety
								+ (Ptmx.this.tileheight - tileN.image.height);
						;
	...
	private class IsometricMap extends Xmap {
		public PVector mapToCanvas(float nx, float ny) {
			float x = (Ptmx.this.mapwidth + Ptmx.this.mapheight) * Ptmx.this.tilewidth / 4
					+ (nx - ny) * Ptmx.this.tilewidth / 2;
		...
		public void drawTileLayer(PGraphics pg, int m) {
			...
			int xstop = parent.min(Ptmx.this.mapwidth,
					parent.ceil(this.canvasToMap(Ptmx.this.camleft + pg.width, Ptmx.this.camtop + pg.height).x
							+ Ptmx.this.drawmargin));
			int ystart = parent.max(0, parent
					.floor(this.canvasToMap(Ptmx.this.camleft + pg.width, Ptmx.this.camtop).y - Ptmx.this.drawmargin));
			int ystop = parent.min(Ptmx.this.mapheight, parent
					.ceil(this.canvasToMap(Ptmx.this.camleft, Ptmx.this.camtop + pg.height).y + Ptmx.this.drawmargin));
			for (int ny = ystart; ny < ystop; ny++)
				for (int nx = xstart; nx < xstop; nx++) {
					n = l.data[nx + ny * Ptmx.this.mapwidth] - 1;
					if (n >= 0) {
	...
	private class StaggeredXMap extends Xmap {
		public PVector mapToCanvas(float nx, float ny) {
			...
			float y = (ny + (float) 0.5 + Ptmx.this.staggerindex / 2) * Ptmx.this.tileheight
					+ (Ptmx.this.staggerindex * 2 - 1) * (parent.abs(parent.abs(nx) % 2 - 1) - 1)
							* (Ptmx.this.tileheight) / 2;
			return new PVector(x, y);
		}
		...
		public PVector canvasToMap(float x, float y) {
			...
			if (Ptmx.this.staggerindex == 0)
				ny = y / Ptmx.this.tileheight - 1 + (parent.abs(parent.abs(nx) % 2 - 1)) / 2;
			else
				ny = y / Ptmx.this.tileheight - (float) 0.5 - (parent.abs(parent.abs(nx) % 2 - 1)) / 2;
			return new PVector(nx, ny);
		}
	...
	private class StaggeredYMap extends Xmap {
		public PVector mapToCanvas(float nx, float ny) {
			...
			float x = (nx + (float) 0.5 + Ptmx.this.staggerindex / 2) * Ptmx.this.tilewidth
					+ (Ptmx.this.staggerindex * 2 - 1) * (parent.abs(parent.abs(y) % 2 - 1) - 1) * (Ptmx.this.tilewidth)
							/ 2;
			return new PVector(x, y);
		}
		...
		public PVector canvasToMap(float x, float y) {
			...
			if (Ptmx.this.staggerindex == 0)
				nx = x / Ptmx.this.tilewidth - 1 + (parent.abs(parent.abs(ny) % 2 - 1)) / 2;
			else
				nx = x / Ptmx.this.tilewidth - (float) 0.5 - (parent.abs(parent.abs(ny) % 2 - 1)) / 2;
			return new PVector(nx, ny);
		}
	...
	private class Layer {
		...
		Layer(String type, String name, boolean visible, float offsetx, float offsety, float opacity, int[] data,
				PImage image, StringDict[] objects, int objcolor, StringDict properties) {
			this.type = type;
			this.name = name;
	...
	private void loadTmxProperties(XML e) {
		...
		this.staggerindex = 0;
		if (e.hasAttribute("staggerindex"))
			if (e.getString("staggerindex").equals("even"))
				this.staggerindex = 1;
		this.staggeraxis = "x";
		if (e.hasAttribute("staggeraxis"))
			this.staggeraxis = e.getString("staggeraxis");
		this.hexsidelength = 0;
		if (e.hasAttribute("hexsidelength"))
			this.hexsidelength = e.getInt("hexsidelength");
		this.backgroundcolor = parent.color(80, 80, 80);
		if (e.hasAttribute("backgroundcolor"))
			this.backgroundcolor = this.readColor(e.getString("backgroundcolor"));
	}
	...
	private void loadTileset(XML e) {
		...
		if (e.hasAttribute("tilecount"))
			tilecount = e.getInt("tilecount");
		if (e.hasAttribute("columns"))
			columns = e.getInt("columns");
		if (e.hasAttribute("spacing"))
			spacing = e.getInt("spacing");
		if (e.hasAttribute("margin"))
			margin = e.getInt("margin");
		...
		if (c != null) {
			...
			if (columns == 0)
				columns = parent.floor((source.width - margin) / (tilewidth + spacing));
			if (tilecount == 0)
				tilecount = columns * parent.floor((source.height - margin) / (tileheight + spacing));
			if (c.hasAttribute("trans"))
				this.applyTransColor(source, c.getString("trans"));
			for (int n = 0; n < tilecount; n++) {
				image = source.get(margin + (n % columns) * (tilewidth + spacing),
						margin + parent.floor(n / columns) * (tileheight + spacing), tilewidth, tileheight);
	...
	private void loadLayer(XML e) {
		...
		if (e.hasAttribute("visible"))
			visible = e.getInt("visible") == 1;
		float offsetx = 0;
		if (e.hasAttribute("offsetx"))
			offsetx = e.getFloat("offsetx");
		float offsety = 0;
		if (e.hasAttribute("offsety"))
			offsety = e.getFloat("offsety");
		float opacity = 1;
		if (e.hasAttribute("opacity"))
			opacity = e.getFloat("opacity");
		int objcolor = parent.color(0);
		if (e.hasAttribute("color"))
			objcolor = this.readColor(e.getString("color"));
		XML c;
		int[] content = null;
		PImage image = null;
		...
		case "imagelayer":
			...
			if (c.hasAttribute("trans"))
				this.applyTransColor(image, c.getString("trans"));
			break;
		case "objectgroup":
			type = "objectgroup";
			...
			for (XML o : xo) {
				...
				if (o.hasAttribute("id"))
					prop.set("id", o.getString("id"));
				if (o.hasAttribute("name"))
					prop.set("name", o.getString("name"));
				if (o.hasAttribute("type"))
					prop.set("type", o.getString("type"));
				if (o.hasAttribute("visible"))
					prop.set("visible", o.getString("visible"));
				if (o.hasAttribute("x"))
					prop.set("x", o.getString("x"));
				if (o.hasAttribute("y"))
					prop.set("y", o.getString("y"));
				if (o.hasAttribute("width"))
					prop.set("width", o.getString("width"));
				if (o.hasAttribute("height"))
					prop.set("height", o.getString("height"));
				if (o.hasAttribute("rotation"))
					prop.set("rotation", o.getString("rotation"));
				if (o.getChild("properties") != null) {
					XML childs[] = o.getChild("properties").getChildren("property");
					for (XML p : childs)
						prop.set(p.getString("name"), p.getString("value"));
				}
		...
		if (e.getChild("properties") != null) {
			...
			for (XML p : childs)
				properties.set(p.getString("name"), p.getString("value"));
		}
		this.layer = (Layer[]) parent.append(this.layer, new Layer(type, name, visible, offsetx, offsety, opacity,
				content, image, objects, objcolor, properties));
	}
	...
	private void applyTransColor(PImage source, String c) {
		...
		for (int p = 0; p < source.pixels.length; p++)
			if (source.pixels[p] == trans)
				source.pixels[p] = parent.color(255, 1);
		source.updatePixels();
	}
		...
		case "objectgroup":
			...
			for (StringDict o : l.objects) {
				if (!o.hasKey("visible")) {
					...
					pg.translate(parent.parseFloat(o.get("x")) - l.offsetx - this.camleft,
							parent.parseFloat(o.get("y")) - l.offsety - this.camtop);
					if (o.hasKey("rotation"))
						pg.rotate(parent.parseFloat(o.get("rotation")) * parent.PI / 180);
					switch (o.get("object")) {
					case "rectangle":
					...
					case "tile":
						...
						if (o.hasKey("rotation"))
							pg.rotate(parent.parseFloat(o.get("rotation")) * parent.PI / 180);
						pg.image(this.tile[parent.parseInt(o.get("gid")) - 1].image,
								this.tile[parent.parseInt(o.get("gid")) - 1].offsetx,
								this.tile[parent.parseInt(o.get("gid")) - 1].offsety, parent.parseFloat(o.get("width")),
								parent.parseFloat(o.get("height")));
						break;
					case "polygon":
					case "polyline":
						if (o.get("object").equals("polyline"))
							pg.noFill();
						pg.beginShape();
		...
		if (pg != parent.g && l.opacity < 1) {// applyOpacity
			...
			for (int p = 0; p < pg.pixels.length; p++)
				if (parent.alpha(pg.pixels[p]) > a)
					pg.pixels[p] = parent.color(parent.red(pg.pixels[p]), parent.green(pg.pixels[p]),
							parent.blue(pg.pixels[p]), a);
			pg.updatePixels();
		}
	}
...
//Layers methods
	...
	public void setVisible(int n, boolean v) {
		if (n >= 0 && n < this.layer.length)
			this.layer[n].visible = v;
	}

	public PImage getImage(int n) {
		if (n >= 0 && n < this.layer.length && this.layer[n].type.equals("imagelayer"))
			return this.layer[n].image;
		else
			return null;
	}

	public StringDict[] getObjects(int n) {
		if (n >= 0 && n < this.layer.length && this.layer[n].type.equals("objectgroup"))
			return this.layer[n].objects;
		else
			return null;
	}
	...
	public int getObjectsColor(int n) {
		if (n >= 0 && n < this.layer.length && this.layer[n].type.equals("objectgroup"))
			return this.layer[n].objcolor;
		else
			return 0;
	}

	public int[] getData(int n) {
		if (n >= 0 && n < this.layer.length && this.layer[n].type.equals("layer"))
			return this.layer[n].data;
		else
			return null;
	}
	...
	public StringDict getCustomProperties(int n) {
		if (n >= 0 && n < this.layer.length)
			return this.layer[n].properties;
		else
			return null;
	}
	...
	public float getOpacity(int n) {
		if (n >= 0 && n < this.layer.length)
			return this.layer[n].opacity;
		else
			return 0;
	}

	public void setOpacity(int n, float o) {
		if (n >= 0 && n < this.layer.length && o >= 0 && o <= 1)
			this.layer[n].opacity = parent.min(parent.max(0, o), 1);
	}

	public int getTileIndex(int n, int x, int y) {
		if (n >= 0 && n < this.layer.length && x >= 0 && y >= 0 && x < this.mapwidth && y < this.mapheight
				&& this.layer[n].type.equals("layer"))
			return this.layer[n].data[x + y * this.mapwidth] - 1;
		else
			return -2;
	}
	...
	public void setTileIndex(int n, int x, int y, int v) {
		if (n >= 0 && n < this.layer.length && x >= 0 && y >= 0 && x < this.mapwidth && y < this.mapheight
				&& this.layer[n].type.equals("layer") && v >= -1)
			this.layer[n].data[x + y * this.mapwidth] = v + 1;
	}
...
//Map methods
	...
	public PVector getCamCorner() {
		if (this.positionmode.equals("MAP"))
			...
			return new PVector(this.camleft, this.camtop);
	}
	...
	public PVector getCamCenter() {
		if (this.positionmode.equals("MAP"))
			...
			return new PVector(this.camleft + this.camwidth / 2, this.camtop + this.camheight / 2);
	}
	...
	public void setCamSize(PVector s) { // Only useful for some pre-draw calculations, since size is always the last
										// PGraphics used.
	...
	public void setDrawMode(int s) {
		if (s == parent.CORNER || s == parent.CENTER)
			this.drawmode = s;
	}
	...
	public void setPositionMode(String s) {
		if (s.equals("CANVAS") || s.equals("MAP"))
			this.positionmode = s;
	}

	public PVector getPosition() {
		if (this.drawmode == parent.CORNER)
			return this.getCamCorner();
		else
			return this.getCamCenter();
	}
	...
	public void setBackgroundMode(String s) {
		if (s.equals("COLOR") || s.equals("CLEAR") || s.equals("NONE"))
...
//Coordinate methods
	...
	public int getTileId(int xs, int ys, int layers) {
		int n;
		float x, y;
		PVector p;
		Tile tileN;
		Layer l = Ptmx.this.layer[layers];
		n = l.data[xs + ys * Ptmx.this.mapwidth] - 1;
		return n;
	}
